...
(function () {
  ...
  D.insertTable = function () {
    ...
    // Spec 034: Word's Insert Table dialog "AutoFit behavior" group (Fixed column width is Word's
    // default insert geometry — only Contents/Window need a follow-up tableAutoFit pass). Lifted from
    // archive 87e63d5. + "Remember dimensions for new tables" (v1 no-op checkbox, present for parity).
    ...
    const autofit = el('div', { style: { marginTop: '10px', borderTop: '1px solid #eee', paddingTop: '8px' } }, [
      ...
      el('label', { class: 'row', style: { gap: '6px', marginTop: '8px', borderTop: '1px solid #eee', paddingTop: '8px' } }, [
        el('input', { type: 'checkbox', id: 'tbl-remember' }),
        el('span', { text: 'Remember dimensions for new tables' }),
      ]),
    ]);
    ...
    function build(rows, cols) {
      ...
      // Fixed column width = the default insert geometry (the bridge already seeds it); Contents/Window
      // need an explicit AutoFit pass.
