...
(function () {
  ...
  // Table Layout > Cell Margins opens Word's "Table Options" dialog (default cell margins + spacing + auto-resize).
  H.tblCellMargins = () => { const p = TPM(); if (!p) return; (WC.Dialogs && WC.Dialogs.tableOptions) ? WC.Dialogs.tableOptions() : WC.toast("Click inside a table cell to set its margins."); };
