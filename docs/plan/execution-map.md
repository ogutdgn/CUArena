...
- [x] **FIX 3 SHIPPED** (`dfbd642`,`c668826`,`8298933`): borders engine — full dropdown + schema/diagonal fix + nil No-Border + pen state + REAL Border Painter + collapse thicker-wins paint (2 plan-authorized fork edits) + B&S dialog cell scope. Both border twins PASS (collapse bug closed). PIPELINE FIX: twin harness reads the painter, not the hidden model host. STRUCTURE table-design 9→14; pm 537/537.
- [x] **FIX 4 SHIPPED** (`ae1607d`,`f1017ea`): Layout tab rebuilt to Word's 7 groups + 7 NO-FORK bridge verbs + 6 dialogs (Sort/Formula/Properties/Options/InsertCells/Convert) + SplitCells. STRUCTURE table-layout 13→34 matched, labels 5→0; OOXML repeatheader/cellalign/cellmargins/sort clean. HONEST: paged-painter vAlign/height/textdir paint = layout-engine follow-up (knowledge note). pm 552/552.
- [x] **FIX 5 SHIPPED** (`60eb24b`): insert-time OOXML defaults + Insert menu wiring — F-class delta cleared across ALL tasks; TB pass 2→17; `table`=semantic-pass; insert journeys pass.
- [x] **FIX 6 SHIPPED** (`de7b77f`,`a29580b`): 3 import losses closed + convert-tab + D1.2 pass-with-note (TB 17→24) + full 6-axis re-measure + acceptance report. Loop COMPLETE.
- [ ] **AWAITING USER:** next = (a) paged-painter paint follow-up, (b) OOXML byte-tail, or (c) Phase C (other features, D2.3 gate first).
- [ ] **FIX 5 (034)**: Insert menu wiring + grid-hover preview + insert-time OOXML defaults (tblLook val/tblW/gridCol/TableGrid spacing — the F-class base delta in every table task).
- [ ] **FIX 4 (033)**: Layout tab completion (Word's 7 groups: Table/Draw/Delete-menu/Insert-Cells/9-way-align/Sort/Formula/Repeat-Header/Convert dialogs).
