...
```json
{
  ...
  "title": "Font",
  "modal": true,
  "entry_points": ["ribbon.home.font.launcher", "dropdowns/…/font-item"],
  ...
  "expansion": null,
  "dependencies": [ { "if": "Line spacing = At least", "enables": ["At:"] } ]
}
```
...
4. **Filenames** are sanitized slugs (lowercase, invalid chars stripped, length-capped, hash
   suffix on collision) — NTFS case-insensitivity and MAX_PATH under the OneDrive path are
...
1. **Frontier empty.** Every node `complete | blocked(reason) | boundary(decision)`. Every
   `opens-*` action has `ref` XOR `boundary`.
