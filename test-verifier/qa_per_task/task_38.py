...
def _battery(n=5, body_radius=8):
    ...
    return make_log([body, terminal, *bars], _events())
...
def fail_4_rects():
    log = perfect()
    log["outcome"]["document"]["pages"][0]["children"].pop()
...
def fail_all_same_color_bars():
    ...
    bars = [make_layer("rectangle", x=220+i*45, y=320, w=40, h=40, fill=(0.5,0.5,0.5))
            for i in range(4)]
    return make_log([body, *bars], _events())
...
FAIL_LOGS = [
    ...
    ("all_same_color_bars",      fail_all_same_color_bars(),      ["≥3"]),
]
