...
def perfect():        return _nested([240, 160, 80], [C1, C2, C1])
def perfect_smaller(): return _nested([180, 120, 60], [C1, C2, C1])
def perfect_other_palette(): return _nested([240, 160, 80], [(0.2,0.4,0.85),(1.0,1.0,1.0),(0.2,0.4,0.85)])


def fail_2_triangles(): return _nested([240, 160], [C1, C2])
def fail_not_concentric():
    cx, cy = 500, 500
    sizes, colors = [240, 160, 80], [C1, C2, C1]
    layers = []
    for i, (sz, c) in enumerate(zip(sizes, colors)):
        layers.append(make_layer("polygon", x=cx-sz/2 + i*50, y=cy-sz/2,
