...
def perfect_log(task) -> dict:
    ...
    # Build layer nodes for non-frame types (frame will be the parent container)
    children = []
    idx = 0
    for stype, n in shapes.items():
        if stype == "frame":
            continue       # don't emit frame as a child; it becomes the parent
        for i in range(n):
            children.append(synth_layer(stype, idx))
            idx += 1

    # Always wrap in a frame so LayerInsideFrame / FrameSizeEquals checks pass.
