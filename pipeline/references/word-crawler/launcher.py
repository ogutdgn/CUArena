...
class WordSession:
    ...
    def start(cls, fixture=None):
        ...
        try:
            ...
            self._move_to_primary()                      # Word can open on a secondary display
    ...
    def _move_to_primary(self):
        """Move the frame onto the PRIMARY monitor before maximizing (Word may open on a secondary
        display — e.g. after the user relaunched Word elsewhere). The primary monitor's origin is
        (0,0) in Windows virtual coordinates, so restore + move to (0,0), then maximize snaps to it.
        Makes the environment pin self-healing instead of failing with 'move it and rerun'."""
        hwnd = self._hwnd()
        try:
