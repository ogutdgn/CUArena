...
def collect_targets(writer):
    """[(surf_id, opener_el_id, host_container, family, owner_node, recurse)] for every
    ribbon-opened stub whose owning node is P0-P2 (recurse) or P3 (one level). family=None
    for Home/Insert; else the contextual object family needed to summon the host tab."""
    layers = load_layers()
    ...
    ribbons = [(cid, c) for cid, c in containers.items()
               if cid in ("ui:ribbon-home", "ui:ribbon-insert") or c.get("trigger_condition")]
    for cid, c in ribbons:
        ...
        for e in c["children"]:
            ...
            if not lay:
                continue
            seen.add(surf)
            targets.append((surf, e["id"], cid, fam, owner, lay == "P0P2"))
    return targets
        ...
        for surf_id, el_id, host, fam, owner, recurse in targets:
            ...
            for attempt in range(3):
                try:
                    ...
                    walker.j("enter", surf_id, f"tab-activate-retry{attempt}: {e}")
                    drv.press_escape(2)
                    walker.close_all()
