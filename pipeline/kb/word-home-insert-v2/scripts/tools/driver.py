...
def click_target(t, settle=0.15):
    """Click a rect (l,t,r,b) at its center OR a point (x,y) — zone_point fallbacks return
    points while measured zones are rects; a caller that assumes one crashes on the other
    (measured: picture-format crawl, 'expected 4, got 2')."""
    t = tuple(t)
    if len(t) == 4:
        return click_rect(t, settle)
    return click_point(t[0], t[1], settle)
