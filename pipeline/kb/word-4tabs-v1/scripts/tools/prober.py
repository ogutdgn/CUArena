...
def snapshot(sess, screen_w):
    return {
        ...
        "layout": sess.layout_fingerprint(),   # Design/Layout effects: page setup + theme
...
def _state_changes(before, after):
    ...
    bl, al = before.get("layout") or {}, after.get("layout") or {}
    if bl and al and bl != al:
        changed.append("layout:" + ",".join(f"{k}{bl.get(k)}->{al.get(k)}"
                                            for k in al if bl.get(k) != al.get(k)))
    return changed
...
def restore(sess, kind, detail, click_pt, baseline, screen_w):
    ...
    if kind in ("dialog", "flyout"):
        ...
        doc_or_fmt = any(x.startswith(("doc", "format", "objects", "layout")) for x in sc)
        ...
        if doc_or_fmt:
            ...
            for _ in range(6):
                ...
                if (sess.doc_hash() == baseline["doc_hash"]
                        ...
                        and sess.layout_fingerprint() == baseline.get("layout")):
                    break
            notes.append("ctrl+z")
        elif app_only:
    ...
    if not ok:
        ...
        layout_drift = after.get("layout") != baseline.get("layout")
        if doc_drift or obj_drift or layout_drift:
            for _ in range(4):
                drv.send_keys("^z")
                time.sleep(0.2)
                if (sess.doc_hash() == baseline["doc_hash"]
