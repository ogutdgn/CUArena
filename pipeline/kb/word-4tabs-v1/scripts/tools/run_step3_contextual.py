...
def object_probes(sess, png_path, svg_path):
    ...
    def ins_header():
        # entering the header STORY is the trigger. Selecting the header Range via COM does NOT
        # switch the window into header-edit mode (so no contextual tab appears) — the view must
        # SeekView into the header (wdSeekCurrentPageHeader=9). That is what activates the
        # Header & Footer tab (measured: word-4tabs-v1 header probe fix).
        app.ActiveWindow.View.SeekView = 9      # wdSeekCurrentPageHeader
        return True

    def sel_header():
        app.ActiveWindow.View.SeekView = 9      # re-enter header-edit mode (the trigger condition)
        return True
...
def main():
    ...
    try:
        ...
        for name, ins, sel, fp, condition, owner in object_probes(sess, png, svg):
            ...
            try:
                sess.app.ActiveWindow.View.SeekView = 0   # wdSeekMainDocument: exit header/footer
            except Exception:
                pass
            try:
