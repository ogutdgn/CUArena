...
class WordSession:
    ...
    def format_sig(self):
        ...
        try:
            ...
            sig = (int(f.Bold), int(f.Italic), int(f.Underline),
                   ...
                   # RightIndent + SpaceBefore/After: the Layout tab's Paragraph spinners
                   # live OUTSIDE the v2 sig — without these their More/Less presses would
                   # measure as false 'no-effect' (same class as highlight/shading,
                   # com.md 2026-07-09)
                   float(p.RightIndent), float(p.SpaceBefore), float(p.SpaceAfter),
                   hl, sh, borders)
    ...
    def object_fingerprint(self):
        ...
        try:
            d = self.doc
            def _shape_states():
                # z-order + wrap type per floating shape: the Layout/contextual Arrange
                # commands (Bring Forward, Send Backward, Wrap Text) change EXACTLY this
                # and nothing else the other fingerprints can see (word-4tabs-v1 step 2)
                try:
                    return tuple(sorted((str(s.Name), int(s.ZOrderPosition),
                                         int(s.WrapFormat.Type)) for s in d.Shapes))
                except Exception:
                    return -1
            return {"tables": _c(lambda: d.Tables.Count),
                    "inline_shapes": _c(lambda: d.InlineShapes.Count),
                    "shapes": _c(lambda: d.Shapes.Count),
                    "shape_states": _shape_states(),
    ...
    def layout_fingerprint(self):
        """Page/section-setup + document-theme fingerprint — the DESIGN and LAYOUT tabs'
        effects live HERE, not in Content.Text or the selection format sig: margins,
        orientation, paper size, columns, hyphenation, line numbering, page color,
        style set / theme. Without these, Design/Layout feature presses measure as
        false 'no-effect' (same failure class as com.md 2026-07-09 highlight/shading;
        pinned for word-4tabs-v1)."""
        def _v(fn, default=-1):
            try:
                return fn()
            except Exception:
        ...
        try:
            ...
            return {"top": _v(lambda: float(ps.TopMargin)),
                    "bottom": _v(lambda: float(ps.BottomMargin)),
                    "left": _v(lambda: float(ps.LeftMargin)),
                    "right": _v(lambda: float(ps.RightMargin)),
                    "orientation": _v(lambda: int(ps.Orientation)),
                    "page_w": _v(lambda: float(ps.PageWidth)),
                    "page_h": _v(lambda: float(ps.PageHeight)),
                    "columns": _v(lambda: int(ps.TextColumns.Count)),
                    "line_numbering": _v(lambda: int(ps.LineNumbering.Active)),
                    "gutter": _v(lambda: float(ps.Gutter)),
                    "vertical_align": _v(lambda: int(ps.VerticalAlignment)),
                    "auto_hyphenation": _v(lambda: int(d.AutoHyphenation)),
                    "hyphenate_caps": _v(lambda: int(d.HyphenateCaps)),
                    "page_color": _v(lambda: int(d.Background.Fill.ForeColor.RGB)),
                    "style_set": _v(lambda: str(d.StyleSheets.Count), "-1"),
                    "spacing_style": _v(lambda: float(d.Styles("Normal").ParagraphFormat.SpaceAfter))}
        except Exception:
            return {}
