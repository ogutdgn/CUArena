# Microsoft Word — Home + Insert + Design + Layout knowledge base

**Version (pinned):** 16.0.20131  |  **Platform:** desktop

A desktop word processor for creating, formatting and editing text documents. Writing and formatting documents — letters, reports, essays, resumes — with rich text styling, paragraph layout, lists, styles, tables, graphics, page setup, themes, headers/footers and review tools.

**Who uses it:** Students, office workers, writers, and virtually anyone producing formatted text documents.

**Contents:** 38 features · 194 sub-features · 408 UI containers (34 honest stubs) · 25 shortcut keys · layers: P0=11 P1=34 P2=27 P3=106 P4=54

## Scope

The HOME, INSERT, DESIGN and LAYOUT ribbon tabs plus every CONTEXTUAL tab their features summon (Table Design/Layout, Picture/Graphics/Shape Format, Chart Design/Format, SmartArt Design/Format, Equation, Header & Footer). Other always-present tabs (References, Mailings, Review, View) are recorded as unexplored names only. The Layout Arrange group is the shared object-arrange machinery (measured with a shape selected). Some nth-level format sub-panes / global-settings (Word Options) / online (Stock/Copilot) openers of medium-priority features are documented as deliberate priority-bounded depth boundaries. The deprecated standalone WordArt tab is out of scope (Insert > WordArt in Word 16 makes a modern text-effect shape whose tab is Shape Format, covered).
...
### Editing  `feature:editing`  — P1 (scope: gems, ratio 2/3)
...
- **Quick Styles gallery** (P1) · opens `ui:quick-styles-gallery` — Applies a named style (Normal, No Spacing, Heading 1/2, Title, Subtitle, Quote…) to the selection from the in-ribbon gallery; the expand arrow opens the full styles flyout.
...
### Document Formatting  `feature:document-formatting`  — P3 (scope: none, ratio 0/7)

Set the document's overall visual theme in one place — theme colors, fonts and effects, the style set, and document-wide paragraph spacing — and save the result as the default.

- **Themes** (P3) · opens `ui:themes-dropdown` — Applies a document theme — a coordinated bundle of theme colors, fonts and effects — to the whole document from a gallery; also browse/reset/save themes.
- **Style Set** (P3) · opens `ui:style-set-gallery` — Applies a style set — a coordinated look for the built-in styles (Title, Headings, body) — from the in-ribbon gallery; also reset to the default or save a new set.
- **Colors** (P3) · opens `ui:theme-colors-dropdown` — Changes the document theme's color scheme from a gallery (or customizes new theme colors).
- **Fonts** (P3) · opens `ui:theme-fonts-dropdown` — Changes the document theme's heading/body font pairing from a gallery (or customizes them).
- **Paragraph Spacing** (P3) · opens `ui:paragraph-spacing-menu` — Applies a document-wide paragraph spacing preset (compact / open / double…) by adjusting the built-in styles' spacing.
- **Effects** (P4) · opens `ui:theme-effects-dropdown` — Changes the document theme's graphic effects (shadow/reflection/line styling for shapes and charts) from a gallery.
- **Set as Default** (P4) · opens `ui:quick-styles-set-as-default-dialog` — Sets the current theme/style-set/spacing formatting as the default for new documents based on this template.
...
### Paragraph (Layout)  `feature:layout-paragraph`  — P3 (scope: none, ratio 0/4)

Set the selected paragraph's precise left/right indentation and the spacing before and after it (the Layout tab's numeric Paragraph group).

- **Indent Left** (P3) — Sets the selected paragraph's left indent to a precise measurement (value field + More/Less steppers).
- **Indent Right** (P3) — Sets the selected paragraph's right indent to a precise measurement (value field + More/Less steppers).
- **Spacing Before** (P3) — Sets the space above the selected paragraph in points (value field + More/Less steppers).
- **Spacing After** (P3) — Sets the space below the selected paragraph in points (value field + More/Less steppers).
...
- **Align** (P3) · opens `ui:ribbon-chart-format-object-align-menu` — Aligns/distributes the selected object(s) to the page/margin/each other (menu).
...
### Page Background  `feature:page-background`  — P3 (scope: none, ratio 0/3)

Decorate the page itself: a watermark behind the content, a page background color, and a border around the page.

- **Watermark** (P3) · opens `ui:watermark-dropdown` — Stamps ghosted text or a picture (DRAFT, CONFIDENTIAL, a logo…) behind the document content on every page, from a gallery or a custom dialog.
- **Page Color** (P4) · opens `ui:page-color-menu` — Fills the page background with a color, gradient, texture or picture (fill effects); chiefly for on-screen/web viewing.
- **Page Borders** (P3) · opens `ui:page-border-and-shading-dialog` — Opens the Borders and Shading dialog on its Page Border tab to draw a decorative or line border around the whole page.

### Page Setup  `feature:page-setup`  — P1 (scope: gems, ratio 3/8)

Set up the printed page — margins, orientation, paper size, text columns, page/section breaks, line numbering and hyphenation.

- **Margins** (P1) · opens `ui:page-margins-dropdown` — Sets the page margins from presets (Normal/Narrow/Wide/Mirrored…) or a custom dialog; the top pick applies immediately.
- **Orientation** (P2) · opens `ui:page-orientation-dropdown` — Switches the page between Portrait and Landscape.
- **Size** (P3) · opens `ui:page-size-dropdown` — Chooses the paper size (Letter, A4, Legal…) from a gallery or a custom dialog.
- **Columns** (P3) · opens `ui:table-columns-dropdown` — Flows the document text into one or more newspaper-style columns (One/Two/Three/Left/Right or a custom dialog).
- **Breaks** (P2) · opens `ui:breaks-menu` — Inserts a page, column, text-wrapping or section break at the cursor from a menu — the control that starts a new page/column/section.
- **Line Numbers** (P4) · opens `ui:line-numbers-menu` — Adds line numbers in the margin (continuous, per-page, per-section) from a menu; used in legal and review documents.
- **Hyphenation** (P4) · opens `ui:hyphenation-menu` — Controls automatic word hyphenation at line ends (None/Automatic/Manual + options).
- **Page Setup dialog launcher** (P3) · opens `ui:page-setup-dialog` — Opens the Page Setup dialog — the consolidated surface for margins, paper, layout (section start, headers/footers distance, vertical alignment) and line numbers.
...
- **Right to Left** (P4) · opens `ui:ribbon-smart-art-design-smart-art-right-to-left-dropdown` — Reverses the left-to-right order of the SmartArt layout.
- **SmartArt Styles gallery** (P3) — Applies a visual style (3D, shadow, gradient…) to the SmartArt from the gallery.
- **Text Pane** (P3) · opens `ui:ribbon-smart-art-design-smart-art-text-dialog` — Toggles the text outline pane for editing the SmartArt's text as a bulleted list.
...
- `ui:ribbon-chart-format` — **Format (contextual tab)**: exists only while an embedded chart is selected (35 controls)
- `ui:ribbon-equation` — **Equation (contextual tab)**: exists only while the cursor is inside an equation (math zone) (20 controls)
- `ui:ribbon-graphics-format` — **Graphics Format (contextual tab)**: exists only while an SVG/icon graphic is selected (27 controls)
- `ui:ribbon-header-footer` — **Header & Footer (contextual tab)**: exists only while header/footer editing mode is active (cursor in header or footer) (24 controls)
- `ui:ribbon-picture-format` — **Picture Format (contextual tab)**: exists only while an inline picture is selected (33 controls)
- `ui:ribbon-shape-format` — **Shape Format (contextual tab)**: exists only while a drawn shape is selected | a text box is selected (36 controls)
- `ui:ribbon-smart-art-design` — **SmartArt Design (contextual tab)**: exists only while a SmartArt graphic is selected (14 controls)
- `ui:ribbon-smart-art-format` — **Format (contextual tab)**: exists only while a SmartArt graphic is selected (34 controls)
- `ui:ribbon-table-design` — **Table Design (contextual tab)**: exists only while selection is inside a table (19 controls)
- `ui:ribbon-table-layout` — **Table Layout (contextual tab)**: exists only while selection is inside a table (39 controls)
...
- `ui:ribbon-smart-art-design-smart-art-right-to-left-dropdown` (dropdown) — Right to Left
...
- `ui:screenshot-insert-dropdown` (dropdown) — Screenshot
...
- `ui:theme-effects-dropdown` (dropdown) — Effects
