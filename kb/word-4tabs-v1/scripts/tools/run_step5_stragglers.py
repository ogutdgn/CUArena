...
def p0p2_stubs():
    """Remaining explored:false containers reachable from a DEPTH-SET node (P0-P3 + whole-scope
    children — the kernel checks all of P0-P3, not just P0-P2), with the opener element id that
    opens each (from the container's entry element)."""
    ...
    for sid, s in subs.items():
        if s.get("parent") in whole:
            p0p2.add(sid)               # whole-scope children, whatever their own layer
