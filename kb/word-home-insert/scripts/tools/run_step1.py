"""Step 1 proof runner: exercise enumerator + driver + capture against the live app.

Scope = Home + Insert. Produces:
  * tools/dumps/home_enum.json + insert_enum.json — every control with name/id(idMso)/type/
    bounds/tooltip/keytip/shortcut per tab.
...
Capture targets are INSERT-tab controls — the new ground this run must prove tools on.
...
def main():
    ...
    try:
        ...
        insert_enum = json.loads((DUMP_DIR / "insert_enum.json").read_text(encoding="utf-8"))

        # --- 2) WHOLE-SURFACE capture with Insert active (last enumerated) ---
        en.select_tab(win, "Insert")
        drv.ensure_frame_foreground(sess.frame)
        time.sleep(0.4)
        frame_img, frame_rect = cap.grab_window(sess.frame)
        frame_rel = writer.save_screenshot(frame_img, "tool:check", "frame-insert")
        ribbon_img = frame_img.crop((0, 0, frame_img.width, 200))
        writer.save_screenshot(ribbon_img, "tool:check", "ribbon-insert")
        jrnl.append(common.journal_event(actor="stage1.toolcheck", action="capture-surface",
                    target="frame(insert)", outcome="ok",
        ...
        icon_checks = []
        for idmso, nm in [("TableInsertGallery", "table"), ("ChartInsert", "chart"),
                          ("HeaderInsertGallery", "header")]:
            rect = _rect_by_id(insert_enum["groups"], idmso)
            if not rect:
        ...
        jrnl.append(common.journal_event(actor="stage1.toolcheck", action="capture-icons",
                    target="ribbon-insert", outcome="ok", data={"icons": icon_checks}))
        summary["icons"] = icon_checks

        # --- 4) DIALOG capture (Insert Chart dialog — local, Cancel-safe) ---
        dlg = _open_capture_close(sess, jrnl, writer, "ChartInsert",
                                  insert_enum["groups"], "dialog", "dialog-insert-chart")
        summary["dialog"] = dlg

        # --- 5) DROPDOWN/FLYOUT capture (Insert Table grid flyout) ---
        win = ua.attach(sess.frame)   # re-attach after dialog
        time.sleep(0.3)
        en.select_tab(win, "Insert")
        enum2 = en.enumerate_tab(win, "Insert")
        fly = _open_capture_close(sess, jrnl, writer, "TableInsertGallery",
                                  enum2["groups"], "flyout", "dropdown-insert-table")
