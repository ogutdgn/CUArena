...
def restore(sess, kind, detail, click_pt, baseline, screen_w):
    ...
    if kind in ("dialog", "flyout"):
        ...
        doc_or_fmt = any(x.startswith("doc") or x.startswith("format") for x in sc)
        app_only = (not doc_or_fmt) and any(x.startswith("app") for x in sc)
        if doc_or_fmt:
            # Ctrl+Z is the universal undo for document AND formatting changes. Re-press does
            # NOT undo a non-toggle apply (font color), so never use it for format resets.
            for _ in range(5):
                drv.send_keys("^z")
                time.sleep(0.2)
    ...
    if not ok:
        ...
        if after["doc_hash"] != baseline["doc_hash"] and "<com-busy>" not in (after["doc_hash"], baseline["doc_hash"]):
            for _ in range(3):
                drv.send_keys("^z")
                time.sleep(0.15)
                if sess.doc_hash() == baseline["doc_hash"]:
