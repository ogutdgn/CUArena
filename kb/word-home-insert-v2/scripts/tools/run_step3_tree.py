...
from ctx_spec import CTX_FEATURES, CTX_SUBS, CTX_EXISTING_ELS, CTX_IDMSO_OVERRIDES, \
    CTX_HONEST_UNEXPLORED, CTX_CLUSTERS, CTX_EXTRA
...
def build_connections():
    ...
    for ids, kind, source, why in CLUSTERS + CTX_CLUSTERS:
        for a in ids:
            for b in ids:
    ...
    for a, b, kind, source, why in EXTRA + CTX_EXTRA:
        add(a, b, kind, source, why)
...
def main():
    ...
    jrnl.append(common.journal_event(actor="stage3.tree", action="decision",
                ...
                data={"reasoning": "auto-generated per-element triggers rewritten to authored "
                      ...
                      "known-capability elements whose effect is not fingerprintable",
    ...
    for s in CTX_SUBS:
        tps = [{"path": ["ui:main-window", tab, e], "kind": "mouse"}
               for (tab, els) in s["hosts"] for e in els]
        if s.get("shortcut"):
    ...
    for f in CTX_FEATURES:
        ...
        ff = {"feature": {
                ...
                "location": hosts[0] if hosts else None,
    ...
    app = {
        ...
        "layout_regions": ["ui:main-window", "ui:ribbon-home", "ui:ribbon-insert"] + ctx_cids,
