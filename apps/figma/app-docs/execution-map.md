...
- **2026-05-10**
  - Panel X/Y/W/H scrub frame-ejection regression fixed. Position/Layout panel transforms now share the same frame containment helper as canvas drag, so moving or resizing a frame child far enough outside dispatches `reparent` and preserves world position instead of leaving the layer clipped/selected as a stale frame child. Numeric scrub for these transform fields wraps live ticks in one transaction and defers containment until pointer-up so the coordinate reference cannot change mid-scrub. Logger schema unchanged — existing `move_layer` / `resize_layer` plus `reorder_layer` events are reused. See `mock_improvement_steps.md` #32.
  - Selection outline doubling in text edit fixed in `SelectionOverlay.tsx`. After `#29` made the canvas `TextEl` invisible during edit, two overlapping selection-blue borders became visible — the SVG bbox outline drawn by `SelectionOverlay`'s "minimal" branch and the `1.5px` CSS border `TextEditor` draws on the `contentEditable` overlay, both at the same world position. Fix: `SelectionOverlay` returns `null` when `editMode.kind === "text"`. Vector / pen_creation sub-modes still receive the SVG outline since their overlays don't draw their own bbox border. Logger / verifier contracts unchanged — render-only suppression. See `mock_improvement_steps.md` #30.
  - Position (X / Y / rotation) and Layout (W / H) glyphs relocated into their `NumericInput` `prefix` slot in `PositionSection.tsx` and `LayoutSection.tsx`. The labels now sit inside each field and double as drag-scrub handles, matching the affordance opacity and corner-radius use in `AppearanceSection`. Previously the labels were outer `<span>`s next to bare inputs, leaving the drag-scrub feature working (via NumericInput's invisible 6px fallback strip) but invisible to the user. `NumericInput` itself untouched — the `prefix` slot already wires pointer-capture for horizontal scrubbing. Logger / verifier contracts unchanged — same `set_transform` / `set_property` events, no new raw targets. See `mock_improvement_steps.md` #31.
  ...
  - Text edit re-entry bug fixed in `move.ts`. After a text layer was created and committed, double-clicking it on the Move tool failed to re-enter text edit mode — the click fell through to drag-arm, so the layer translated instead of re-editing. Root cause: `MouseEvent.detail` click-count is unreliable on spaced `pointerdown` events across browsers. Fix: manual timestamp + world-position double-click detection (Δt < 350ms, Δworld < 5px, same layer); original `e.detail >= 2` fast path retained. Logger and verifier contracts unchanged — `enterTextEdit` already emits the existing `mode_change` semantic event, edit mode is UI state and is not part of `outcome.document`. See `mock_improvement_steps.md` #28.
  - Text edit overlay double-render bug fixed in `NodeRenderer.tsx`. On re-entering text edit, the SVG `TextEl` stayed visible behind the `TextEditor` `contentEditable` overlay — both painted the same glyphs at the same screen position, producing a ghosted double-text artifact. Fix: `TextEl` now subscribes to `editMode` and returns `null` when the layer is being edited; the overlay becomes the sole renderer until commit/escape. Logger and verifier contracts unchanged — render gating reads UI state, document/outcome content is untouched. See `mock_improvement_steps.md` #29.
