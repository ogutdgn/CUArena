...
Bezier handles required at each anchor (single continuous path).
"""
...
from verifier.checks.stroke_checks import (
    StrokeWeightEquals, StrokeIsDashed,
...
task = Task(
    ...
    description="1 pen-tool S-curve with bezier handles + thick (12px) dashed stroke (ribbon).",
    rubrics=[
        # critical: pen-drawn vector required (exactly 1, not many)
        FundamentalsRubric([
            ShapeCount("vector", equals=1),    # 0 ★ prompt: "single continuous path"
        ], weight=0.25, critical=[0]),

        # critical: bezier handles + non-degenerate, upright, not flipped, visible.
        AlignmentRubric([
            VectorsCurvedCountAtLeast(minimum=1),                                       # 0 ★ prompt: "bezier handles at each anchor"
            LayerSizeAtLeast(layer_type="vector", min_w=50, min_h=20),                  # 1 no degenerate (S-curve must have actual extent)
            LayerShortDimensionAtMost(layer_type="vector", max_value=1500),             # 2 not absurd
            LayerRotationEquals(layer_type="vector", degrees=0, tolerance=15.0),        # 3 roughly upright
            NoLayerFlipped(layer_type="vector"),                                        # 4 no mirror
            LayerVisible(layer_type="vector"),                                          # 5 visible
        ...
        ColorRubric([
            ...
            AllStrokeWeightAtMost("vector", max_weight=25.0),                   # 3 stroke not absurdly thick
