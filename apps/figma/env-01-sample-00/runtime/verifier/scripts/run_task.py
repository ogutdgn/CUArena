...
def resolve_session_id(host: str, port: int, requested_session_id: str | None) -> str | None:
    """Resolve target session id.

    If --session-id is provided, use it. Otherwise pick the newest session id
    reported by /dev-log/status.
    """
    if requested_session_id:
        return requested_session_id
    status = fetch_log_status(host, port, session_id=None) or {}
    for key in ("lastSessionId", "selectedSessionId"):
...
def cmd_full_pipeline(task_input: str, host: str, port: int, session_id: str | None = None) -> None:
    ...
    if session_id is None and resolved_session_id:
        print(f"Auto-selected latest sessionId: {resolved_session_id}")
    ...
    try:
        ...
        status = fetch_log_status(host, port, session_id=resolved_session_id)
        if status:
            print(
                "Relay status:"
