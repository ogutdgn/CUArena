...
def fail_one_hex_filled():
    log = _web()
    children = log["outcome"]["document"]["pages"][0]["children"][0]["children"]
    polygons = [c for c in children if c["type"] == "polygon"]
    polygons[1]["fills"] = [{"kind": "solid", "color": {"r": 1, "g": 1, "b": 1, "a": 1}, "opacity": 1, "visible": True}]
    return log
def fail_one_hex_gradient_filled():
    log = _web()
    children = log["outcome"]["document"]["pages"][0]["children"][0]["children"]
    polygons = [c for c in children if c["type"] == "polygon"]
    polygons[1]["fills"] = [{"kind": "gradient", "opacity": 1, "visible": True}]
    return log
...
FAIL_LOGS = [
    ...
    ("one_hex_filled",     fail_one_hex_filled(),     ["visible fill"]),
    ("one_hex_gradient_filled", fail_one_hex_gradient_filled(), ["visible fill"]),
]
