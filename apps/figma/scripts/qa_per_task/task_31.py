...
def fail_rays_not_radial():
    ...
    frame = make_frame(layers, w=900, h=900)
    return make_log([frame], _events())
def fail_rays_away_from_circle():
    log = _sun()
    ellipse = log["outcome"]["document"]["pages"][0]["children"][0]["children"][0]
    ellipse["x"] += 180
    ellipse["y"] += 120
    return log
...
FAIL_LOGS = [
    ...
    ("rays_away_from_circle", fail_rays_away_from_circle(), ["polygon centroid"]),
]
