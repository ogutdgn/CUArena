...
scripts/run_task.py <task>
  ...
  │  Writes the log to verifier/logs/<task>_<timestamp>.json
  │  Imports the matching tasks/<task>.py module and runs its rubrics
  │  Writes score to verifier/scores/<task>_<timestamp>.json
...
verifier/.venv/Scripts/python scripts/run_task.py task_01_house_task_comprehensive

# Just export the log, no scoring
verifier/.venv/Scripts/python scripts/run_task.py export-log
verifier/.venv/Scripts/python scripts/run_task.py export-log task_01
```

The log lands in `verifier/logs/<resolved-task>_<timestamp>.json` and the
score in `verifier/scores/<resolved-task>_<timestamp>.json`.

> Use the verifier venv's python (`verifier/.venv/Scripts/python` on
> Windows, `verifier/.venv/bin/python` on Unix). The script imports the
