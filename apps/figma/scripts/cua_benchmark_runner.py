...
def _trace_print(enabled: bool, prefix: str, message: str) -> None:
    if not enabled:
        return
...
class OpenAIComputerAgent:
    ...
    async def run_episode(self, harness: BrowserHarness, task_prompt: str, episode_dir: Path) -> EpisodeTrace:
        ...
        deadline = time.monotonic() + self.max_episode_seconds if self.max_episode_seconds > 0 else None
        ...
        turns = 1
        _trace_print(self.live_trace, self.trace_prefix, "turn=1 response received")

        while True:
            if has_step_limit and steps >= self.max_steps:
                break
            if deadline and time.monotonic() >= deadline:
                stop_reason = "max_episode_seconds_reached"
                _trace_print(self.live_trace, self.trace_prefix, f"stopping: {stop_reason}")
                break
            ...
            if not computer_call:
                ...
                _trace_print(self.live_trace, self.trace_prefix, f"model_stop: {stop_reason}")
                break
            ...
            for action_idx, a in enumerate(actions, start=1):
                if has_step_limit and steps >= self.max_steps:
                    break
                if deadline and time.monotonic() >= deadline:
                    stop_reason = "max_episode_seconds_reached"
                    _trace_print(self.live_trace, self.trace_prefix, f"stopping: {stop_reason}")
                    break
                await harness.execute_openai_actions([a])
                steps += 1
                _append_jsonl(actions_path, {"ts": _utc_now(), "provider": "openai", "action": a})
                last_screenshot_b64 = await harness.screenshot_b64()
                shot_path = screens_dir / f"turn_{turns:03d}_action_{action_idx:02d}.png"
                shot_path.parent.mkdir(parents=True, exist_ok=True)
                shot_path.write_bytes(base64.b64decode(last_screenshot_b64))
                _trace_print(self.live_trace, self.trace_prefix, f"step={steps} action={a.get('type', 'unknown')}")
            if stop_reason == "max_episode_seconds_reached":
                break

            if last_screenshot_b64 is None:
                stop_reason = "no_executed_actions"
                break
            follow = {
                "model": self.model,
                "tools": [self._tool_def()],
                ...
                "input": [
                    {
                        ...
                        "output": {
                            ...
                            "image_url": f"data:image/png;base64,{last_screenshot_b64}",
                            "detail": "original",
                        },
                    }
                ],
            ...
            _trace_print(self.live_trace, self.trace_prefix, f"turn={turns} response received")

        hit_max_steps = has_step_limit and steps >= self.max_steps
...
class AnthropicComputerAgent:
    ...
    async def run_episode(self, harness: BrowserHarness, task_prompt: str, episode_dir: Path) -> EpisodeTrace:
        ...
        while True:
            ...
            _trace_print(self.live_trace, self.trace_prefix, f"turn={turns} stop_reason={stop_reason}")
            _append_jsonl(
                messages_path,
                {
                    "ts": _utc_now(),
            ...
            if not tool_uses:
                stopped_by_model = True
                _trace_print(self.live_trace, self.trace_prefix, "model_stop: no_tool_use")
                break
            ...
            for action_idx, block in enumerate(tool_uses, start=1):
                ...
                _trace_print(self.live_trace, self.trace_prefix, f"step={steps} action={inp.get('action', 'unknown')}")
                if has_step_limit and steps > self.max_steps:
                    break
...
async def _run_one_episode(
    ...
    trace_prefix = f"[{provider} task_{task_id_2d} r{run_index:03d}]"
    _trace_print(cfg.live_trace, trace_prefix, f"run_dir={ep_dir}")
...
def _load_config(args: argparse.Namespace) -> RunnerConfig:
    ...
    live_trace = bool(args.live_trace)
    if args.observer:
        max_steps = 0
        live_trace = True
    ...
    return RunnerConfig(
        ...
        width=args.width,
        height=args.height,
        max_episode_seconds=max(0, int(args.max_episode_seconds)),
...
async def _amain(args: argparse.Namespace) -> int:
    ...
    if cfg.max_steps < 0:
        raise SystemExit("--max-steps must be >= 0 (0 means no step cap)")
    if cfg.max_episode_seconds < 0:
        raise SystemExit("--max-episode-seconds must be >= 0")
...
def main() -> None:
    ...
    p.add_argument("--max-steps", type=int, default=80, help="max tool actions per episode (0 disables step cap)")
    p.add_argument("--max-episode-seconds", type=int, default=1800, help="wall-clock safety cap per episode (0 disables)")
    p.add_argument("--live-trace", action="store_true", help="stream per-turn/per-action trace to stdout")
    p.add_argument("--observer", action="store_true", help="convenience mode: --max-steps 0 and --live-trace")
