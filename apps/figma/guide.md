...
## Observer mode (natural run, safety watchdog)
Use this when you want effectively unbounded behavior but still avoid forever loops:
...
.venv/bin/python scripts/cua_benchmark_runner.py \
  ...
  --runs-per-task 1 \
  --observer \
  --max-episode-seconds 1800 \
  --max-parallel 1 \
  --output-dir scripts/cua_runs/observer_task01
```

What it does:
- disables step cap (`--max-steps 0`)
- streams live trace to stdout (turn/action events)
- keeps full artifacts + per-action screenshots in run folders
- enforces only a wall-clock watchdog (`max_episode_seconds`)
