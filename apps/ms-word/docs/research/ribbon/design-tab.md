...
> several **material corrections** (LO *does* ship a preset theme-color gallery; the theme color editor is a separate
...
|---|---|---|---|---|---|---|
| Document Formatting group | GroupStyleSet | group | — | differs | Our-layer UI | Ribbon group container (left, larger of the two Design groups) holding Style Set, Themes, Colors, Fonts, Paragraph Spacing, Effects, and Set as Default. — **LO:** No matching rich group. **(re-verify: prior cite "LO's Design tab is a SINGLE flat toolbox `Design-Section-Main`/`DesignToolBox`" was to the removed project-custom `notebookbar_cua.ui`. Stock `notebookbar.ui` differs materially — it has NO Design tab at all (tabs: File/Home/Insert/Layout/References/Review/View/Table/Image/Draw/Object/Media/Print/Form/Extension/Tools), so no `Design-Section-Main`/`DesignToolBox` flat toolbox exists, and `.uno:DesignerDialog`/`.uno:LoadStyles` are absent from `notebookbar.ui` entirely. The remaining buttons are scattered across other tabs: Watermark + PageDialog on **Layout** (`notebookbar.ui:6296,6918`), ParaspaceIncrease/Decrease on **Home** (`:4063,4075`), BorderDialog on **Image/Object** (`:11201,13885`). The "single flat toolbox" was a property of the cua-custom ribbon, not of stock LO — flagged rather than guessed.)** The visible Word group also merges what the idMso schema splits across GroupStyleSet and a separate GroupThemesWord (see QA flags). |
...
| Word control | idMso | type | LO .uno: | verdict | work | note |
|---|---|---|---|---|---|---|
| Page Background group | GroupPageBackground | group | — | differs | Our-layer UI | Ribbon group container (right side of the Design tab) holding Watermark, Page Color, and Page Borders. — **LO:** no "Page Background" sub-group exists. The relevant commands DO exist as LO commands — `.uno:Watermark`, `.uno:PageDialog` ("~Page Style…"), and `.uno:BorderDialog` — and `.uno:BackgroundColor` is a Home-tab control. **(re-verify: prior cite "LO's Design tab is one flat toolbox" was to the removed `notebookbar_cua.ui`. Stock `notebookbar.ui` has no Design tab; Watermark + PageDialog sit on the **Layout** tab (`notebookbar.ui:6296,6918`), BorderDialog on the **Image/Object** tabs (`:11201,13885`), BackgroundColor on **Home** (`:4098`) — the ribbon placement, not the command existence, is what changed.)** |
...
| Page Borders | PageBorderAndShadingDialog | button (dialog launcher) | `.uno:PageDialog` | differs | Our-layer UI | Opens Borders and Shading on the Page Border tab — choose a Setting (None/Box/Shadow/3-D/Custom), line Style/Color/Width or an Art decorative border, toggle edges in Preview, scope with Apply to; OK applies a border around the page margins. — **LO:** **Corrected:** page-scoped borders are set in the Page Style dialog's Borders tab (`.uno:PageDialog` → Borders): arrangement, style, width, color, spacing, shadow. The `.uno:BorderDialog` button ("Borders") is the PARAGRAPH/object border dialog (selection-scoped), NOT the page border; the page border lives in `.uno:PageDialog` → Borders. **(re-verify: prior cite "`.uno:PageDialog` is ALSO present on the same flat Design toolbox, so the page-border path does not require leaving the tab" was to the removed `notebookbar_cua.ui`. Stock `notebookbar.ui` has no Design tab and the two are not co-located — `.uno:PageDialog` is on the **Layout** tab (`notebookbar.ui:6918`), `.uno:BorderDialog` on the **Image/Object** tabs (`:11201,13885`).)** Differences: LO has NO decorative Art (clip-art) page borders, and no combined "Borders and Shading" dialog (page border + page background fill are separate tabs of the page-style dialog). ✓ verified vs LO source. |
...
earlier hacked/stripped tree) and **override** the mapped rows where they conflicted.
...
> **Re-vendor caveat (2026-05).** This doc's ribbon-placement evidence was originally cited to a
> **project-custom `sw/uiconfig/swriter/ui/notebookbar_cua.ui`** that had a bespoke "Design" tab.
> The engine was re-vendored to PRISTINE LibreOffice; **`notebookbar_cua.ui` no longer exists.** The
> stock ribbon is `sw/uiconfig/swriter/ui/notebookbar.ui`, which **has no Design tab at all** and
> therefore no flat "Design toolbox." Every `notebookbar_cua.ui:NNNN` citation below has been
> re-checked against stock `notebookbar.ui` and either re-anchored or flagged. **The `.uno:`
> command facts (commands, labels, slots, dialog `.ui` contents) were verified against
> command/dialog sources — `WriterCommands.xcu`, `GenericCommands.xcu`, `themedialog.ui`,
> `watermarkdialog.ui`, etc. — not the notebookbar, so those are unaffected by the re-vendor.**
> Only the *which-ribbon-tab-hosts-the-button* claims were `notebookbar_cua.ui`-dependent.

**Material corrections (CORRECTED):**

- **Design-tab structure (group membership)** — the inventory described a "Document Formatting"
  group and a "Page Background" group sourced from a `ribbon.json`. **No `ribbon.json` exists.**
  **(re-verify: the prior cite claimed "the real project ribbon is `notebookbar_cua.ui`, whose
  Design tab is a SINGLE flat toolbox `Design-Section-Main`/`DesignToolBox`," with line citations
  `notebookbar_cua.ui:5726-5841,5791,5801,5811,9259,3839`. That project-custom file was REMOVED by
  the re-vendor. Stock `notebookbar.ui` differs structurally: it has NO "Design" tab — the 16 tabs
  are File/Home/Insert/Layout/References/Review/View/Table/Image/Draw/Object/Media/Print/Form/
  Extension/Tools — so there is NO `Design-Section-Main`/`DesignToolBox` flat toolbox, and
  `.uno:DesignerDialog` and `.uno:LoadStyles` do not appear in `notebookbar.ui` at all.)** Where the
  individual buttons DO appear in stock `notebookbar.ui`, re-anchored: `.uno:Watermark` →
  `Layout-Watermark` on the **Layout** tab (`notebookbar.ui:6296`; also a `MenuPage-Watermark` menu
  item at `:1318`), `.uno:PageDialog` → `Layout-PageDialog` on **Layout** (`:6918`),
  `.uno:BorderDialog` on the **Image/Object** tabs (`:11201,:13885`),
  `.uno:ParaspaceIncrease`/`.uno:ParaspaceDecrease` on the **Home** tab (`:4063,:4075`),
  `.uno:ChangeTheme` ("Dark Mode") as `View-DarkMode` on the **View** tab (`:9007`),
  `.uno:BackgroundColor` on the **Home** tab (`:4098`). `.uno:ThemeDialog` appears as a
  `MenuPage-Theme` menu item (`:1325`). The substantive Word-vs-LO conclusion is unchanged (Word's
  Design tab has no stock-LO ribbon-tab equivalent; the commands exist but are not co-located on a
  Design tab); only the cua-ribbon "single flat Design toolbox" framing and its line citations were
  invalidated.
...
- **Page Borders — BorderDialog vs PageDialog** — `.uno:BorderDialog` is "Borders"
  (selection-scoped paragraph/object border); `.uno:PageDialog` is "~Page Style…" (page-scoped,
  Borders tab). Both commands exist and the command/label facts hold. **(re-verify: the prior cite
  "ALSO on the same flat Design toolbox, so the page-border path is present on the tab" was to
  `notebookbar_cua.ui:5811-5813,5801-5803`; that custom Design toolbox is gone. In stock
  `notebookbar.ui` the two are NOT co-located: `.uno:BorderDialog` is on the **Image/Object** tabs
  (`notebookbar.ui:11201,13885`) and `.uno:PageDialog` is on the **Layout** tab (`:6918`) — so the
  "same tab" claim no longer holds in stock LO.)** Command evidence (re-vendor-unaffected):
  `WriterCommands.xcu:1542-1545,1571-1574`.
- **Paragraph Spacing** — `.uno:ParaspaceIncrease` ("Increase" / "Increase Paragraph Spacing") and
  `.uno:ParaspaceDecrease` are the spacing-nudge commands; line-spacing commands
  `.uno:SpacePara1/115/15/2` ("Line Spacing: 1/1.15/1.5/2") exist. No paragraph-spacing PRESET-menu
  command exists. Evidence (command sources, re-vendor-unaffected): `GenericCommands.xcu:160-187,1995-2022`.
  **(re-verify: the prior cite called these "the Design buttons" per `notebookbar_cua.ui:5771-5783`;
  that Design toolbox is gone. In stock `notebookbar.ui` `.uno:ParaspaceIncrease`/`.uno:ParaspaceDecrease`
  are on the **Home** tab at `notebookbar.ui:4063,4075`, not a Design tab — re-anchored.)**
- **Reset to Default Style Set → `.uno:LoadStyles`** — label "~Load Styles from Template" confirmed.
  The dialog-level "Overwrite" option is now **confirmed present** (the earlier UNCERTAIN was due to
  the stripped tree; the pristine re-vendor includes the dialog `.ui`): `loadtemplatedialog.ui:343-353`
  has an `overwrite` GtkCheckButton labelled "_Overwrite" — "Replaces styles in the current document
  that have the same name as the styles you are loading." Evidence: `WriterCommands.xcu:1524-1530`;
  `sfx2/uiconfig/ui/loadtemplatedialog.ui:343-353`.
...
| Design-tab group membership sourced from "ribbon.json"? | **Resolved (LO source) — REVERSED; ribbon cite re-verify** | No ribbon.json exists. **(re-verify: this resolution cited "the real ribbon is `notebookbar_cua.ui`; the Design tab is a single flat toolbox …"; that project-custom file was removed by the re-vendor. Stock `notebookbar.ui` has NO Design tab and no flat Design toolbox — see the "Design-tab structure" bullet above. The Word-side "no rich group" conclusion is unaffected, but the LO-ribbon evidence is now the stock per-tab placement, not a single Design toolbox.)** |
