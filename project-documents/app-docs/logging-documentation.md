...
All three streams are mirrored from in-memory ring buffers + the live store into `sessionStorage` on a 250ms throttle ([`test-app/src/logger/persist.ts`](test-app/src/logger/persist.ts)).
...
**Export:** [`test-app/src/logger/export.ts`](test-app/src/logger/export.ts) bundles all three into one JSON file (`figma-mock-log-${sessionId}.json`) for offline scoring.

---

## Raw stream

Captures every relevant DOM input event before any handler can stop it. Source: [`test-app/src/logger/raw.ts`](test-app/src/logger/raw.ts).
...
User-intent events emitted by the engine when a meaningful operation happens (e.g. shape created, layer moved, undo, page switch). Source: [`test-app/src/types/events.ts`](test-app/src/types/events.ts), emission via [`test-app/src/logger/semantic.ts`](test-app/src/logger/semantic.ts).
...
A single snapshot of "what got built" — the live document plus a small precomputed summary. Overwritten on every flush; **no history kept**. Source: [`test-app/src/logger/outcome.ts`](test-app/src/logger/outcome.ts).
...
Type: `DocumentNode` from [`test-app/src/types/scene.ts`](test-app/src/types/scene.ts).
