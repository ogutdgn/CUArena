...
This skill will document the protocol the user used to build `apps/figma/helper/` so it can be repeated for Sheets and Docs.
...
2. **Raw fetch**: download into a temporary local folder (NOT committed to git). Path convention: `apps/<app>/helper/<source>_raw/` — added to `.gitignore` immediately.
...
4. **Final extraction**: produce the committed corpus under `apps/<app>/helper/extracted/` and `apps/<app>/helper/analysis/`. Each piece must trace back to its raw source via citations or section refs.
5. **Index**: write `apps/<app>/helper/00-overview.md` as the single entry point. Every other helper file is reached through it. (See `helper-blind-read-prevent` skill.)
...
The figma corpus is the working example: `apps/figma/helper/00-overview.md` shows the final structure.
