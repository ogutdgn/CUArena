...
> **Status (current):** Planning + extraction phases are complete. Code lives under `test-app/`. The current source of truth for what to build is `feature-checklist.md` + `execution-map.md` (per `CLAUDE.md`). This plan is kept as historical context; the closed feature list and Tier-2-only language below has been **superseded** by the broader research output under `helper/extracted/`.
...
- **Code lives in `test-app/`** (Vite + React + TS).
...
| **3. Engine + first slice** | Vite + React + TS app under `test-app/`; canvas + selection + shapes + transform + clipboard + history scaffolded. | `test-app/` | 🔄 in progress |
| **4. Feature-by-feature slicing** | Iterate per `execution-map.md`. | Incremental commits in `test-app/` | 🔄 in progress (recent: frame nesting, drag-into-frame, parent-bounds overlay) |
...
| **Primary agent (me)** | Orchestrate, integrate, build | builds in `test-app/`, references `helper/extracted/` |
...
└── test-app/                       Vite + React + TS application
    └── ARCHITECTURE.md             engine, scene graph, ops, logger, UI shell
```

## 7a. How to use this material — workflows for implementation agents

This section is the **practical entry guide** for an agent (or human) implementing a feature in `test-app/`. It says *which file to read first*, *which to read next*, and *what to do at each branch*. It is not a summary of `helper/` contents (that's §7 above and `01-…` / `02-…`); it's a how-to.
...
4. Final taxonomy is consolidated incrementally as features ship — see `test-app/ARCHITECTURE.md` for the live registry.
...
| "How does the engine work?" | `test-app/ARCHITECTURE.md` |
...
- **Tech stack:** Vite + React + TS (under `test-app/`).
