...
SECTION_KINDS = {"gallery","menu-items","color-grid","controls","form","size-grid"}
...
def validate_control(d):
    ...
    # a split control must carry BOTH zones, UNLESS it is boundary-declared (never zone-probed)
    if d.get("type") == "split":
        has_zones = bool(d.get("primary")) and bool(d.get("flyout"))
        is_boundary = bool(d.get("action", {}).get("boundary"))
        if not (has_zones or is_boundary):
            errs.append(f"{d['id']}: split missing primary/flyout zones")
    if "action" in d:
...
def validate_popup(d):
    ...
    for s in d.get("sections", []):
        ...
        if kind not in SECTION_KINDS:
            errs.append(f"{d['id']}: bad section kind {kind!r}")
        for it in s.get("items", []):
            if not it.get("id"):
                errs.append(f"{d['id']}: item id missing ({it.get('label')})")
            if kind == "color-grid" and "rgb" not in it and "bounds" not in it:
                errs.append(f"{d['id']}: color-grid item {it.get('id')} has no rgb/bounds")
            if "action" in it:
