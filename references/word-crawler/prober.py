...
def _has_close_pane(win):
    """A 'Close pane' button INSIDE the frame = a task pane is open. This catches FLOATING panes
    (e.g. Styles) that don't inset the document area, which the _WwG geometry signal misses
    (FIX_PLAN E2). Bounded to the frame rect so a Windows notification/taskbar 'close' elsewhere
    can't false-trigger. Baseline is clean (full_run closes panes before each control)."""
    try:
        fr = win.rectangle()
        fbox = (fr.left, fr.top, fr.right, fr.bottom)
        for b in win.descendants(control_type="Button"):
            if "close pane" in (b.element_info.name or "").lower():
                r = b.element_info.rectangle
                if fbox[0] - 3 <= r.left and r.right <= fbox[2] + 3 \
                        and fbox[1] - 3 <= r.top and r.bottom <= fbox[3] + 3:
                    return True
    except Exception:
        pass
    return False
...
def classify(before, after):
    ...
    if any(_is_dialog_class(w[0]) for w in new_wins):
        return "dialog"
    # A toggle-state change is DEFINITIVE evidence the control itself is a toggle, so it must beat a
    # COINCIDENT transient popup-class window (tooltip / live-preview) that intermittently appears
    # during observe. Checking popup first misclassified real toggles (aligncenter/subscript/
    # superscript) as dropdown-openers (bug #2). Genuine dropdown/menu openers have no toggle state,
    # so they fall through to the popup check below and are unaffected.
...
def probe(session, win, journal, control_el, control_id, zone=None, split_children=None):
    ...
    # floating-pane rescue: a control the geometry signal read as feature/unresolved but which
    # actually opened a task pane (Styles pane floats, doesn't inset the doc) -> reclassify (E2).
    if cls in ("unresolved", "feature") and _has_close_pane(win):
        cls = "pane"
    ...
    outcome = {"t": "press-outcome", "control": control_id, "kind": cls, "ref": ref}
    if cls == "popup":
        # diagnostic: record the popup window(s) so a transient tooltip/live-preview mistaken for a
        # flyout on ANOTHER tab can be identified and filtered without a repro hunt (bug #2 class).
        outcome["popup_windows"] = [{"class": c, "title": (t or "")[:40], "area": (r[2] - r[0]) * (r[3] - r[1])}
                                    for (h, c, t, r) in after.popup_els if h in after.popups
                                    and h not in before.popups]
