...
| **Verifier file** | `test-verifier/tasks/<task_name>.py` | Python file that scores agent runs |
...
Copy the template into `test-verifier/tasks/<task_name>.py`:
...
What's available in `test-verifier/verifier/checks/`:
...
If the check is reusable across tasks, contribute it back to `test-verifier/verifier/checks/<category>.py` so the next task can import it.
...
**File**: `test-verifier/tasks/house_task_comprehensive.py` (~70 lines, see file).
...
- **Action log is brittle to schema changes**: when test-app adds a new semantic event name, existing event checks may break. Test new tasks against a sample log first.
...
```bash
# Add a CSV row, then:
cp test-verifier/tasks/house_task_comprehensive.py test-verifier/tasks/<task_name>.py
