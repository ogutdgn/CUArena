...
- `cohesion` — the group's NATURE, measured (tests in playbook 03): **`capability`** = children are facets of ONE capability (Font, Table Layout); **`catalog`** = children are independent capabilities that merely share a drawer (Illustrations). Judged per feature, never inherited — a catalog's child may summon its own world that is itself a capability feature (Illustrations → insert-pictures → Picture Format). Drives replication scope: a catalog never replicates WHOLE (playbook 04 R4.7).
...
3. **Connections** — between any two nodes at any level (feature↔feature, sub↔sub, cross-level). Three kinds, strongest first: **`requires`** (directional — B does not work without A; measured via contextual discovery, disabled-state preconditions, and artifact relationships), **`affects-same`** (both shape the same artifact; measured from press-observe state-diff targets), **`co-location`** (the app placed them together; weakest, hint only). Connections serve **logistics, not value**: `requires` edges power replication closure (membership at "enough to work" depth — never a layer change); edge counts are NOT a popularity/value score.
...
Features are never scored independently: a feature's layer = its best child's layer, plus a ratio (children in P0–P3) that sets replication scope — majority high → replicate the feature WHOLE (capability-cohesion features only; a CATALOG never goes whole, its children are judged independently); isolated gems → only the gems + their closure. Then compute **closure**: the replication set = P0–P3 plus everything reachable via `requires` edges (pulled-in nodes get "enough to work" depth, labeled `pulled-in-by`). Connection density is NOT a value signal.
