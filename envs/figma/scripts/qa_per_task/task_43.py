...
from qa_per_task._helpers import (
    make_layer, make_frame, make_log, make_event, SAND, GOLD, RED,
)
...
def _compass(triangle_colors=None):
    ...
    for i, color in enumerate(triangle_colors):
        ...
        tx = cx + 100 * math.cos(angle) - 15
        ty = cy + 100 * math.sin(angle) - 50
        triangles.append(make_layer("polygon", x=tx, y=ty, w=30, h=100, fill=color,
                                     sides=3, rotation=angle_deg))
    return _wrap([base, pivot, *triangles])
...
def perfect_smaller():
    ...
    for i in range(4):
        angle_deg = i * 90
        angle = math.radians(angle_deg)
        tx = cx + 70 * math.cos(angle) - 10
        ty = cy + 70 * math.sin(angle) - 35
        triangles.append(make_layer("polygon", x=tx, y=ty, w=20, h=70,
...
def fail_wrong_rotation_step():
    ...
    for i in range(4):
        ...
        angle = math.radians(angle_deg)
        triangles.append(make_layer("polygon", x=cx+100*math.cos(angle)-15,
                                     y=cy+100*math.sin(angle)-50, w=30, h=100,
...
FAIL_LOGS = [
    ...
    ("uniform_triangles",     fail_uniform_triangles(),     ["distinct polygon colors"]),
]
